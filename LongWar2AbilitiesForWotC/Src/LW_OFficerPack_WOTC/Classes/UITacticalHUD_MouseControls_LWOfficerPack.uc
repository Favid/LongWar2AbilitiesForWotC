...
simulated function UpdateControls()
{
	...
	else
	{
		...
		for(i = 0; i < CommandAbilities.Length; i++)
		{
			...
			SetButtonItem( m_optCallSkyranger + i,    label,   key != "" ? key : m_strNoKeyBoundString,    ButtonItems[m_optCallSkyranger + i].UIState, BattleData.IsAbilityObjectiveHighlighted(AbilityState.GetMyTemplate())); // Previously AbilityState.GetMyTemplateName()
		}
