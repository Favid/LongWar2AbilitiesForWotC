...
class XComGameState_Effect_LW2WotC_IncomingReactionFire extends XComGameState_Effect_EffectCounter config (LW_SoldierSkills);
...
function XComGameState_Effect_LW2WotC_IncomingReactionFire InitFlyoverComponent()
{
...
function XComGameState.EventListenerReturn TriggerLRFlyover(Object EventData, Object EventSource, XComGameState GameState, name EventID, Object CallbackData)
{
    ...
    if (DefendingUnit != none)
    {
        if (DefendingUnit.HasSoldierAbility('LW2WotC_LightningReflexes'))
        {
            if (!FlyoverTriggered)
            {
                ...
                ThisEffect=XComGameState_Effect_LW2WotC_IncomingReactionFire(NewGameState.CreateStateObject(Class,ObjectID));
